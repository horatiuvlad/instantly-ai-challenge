...
const routes: FastifyPluginAsync = async (fastify): Promise<void> => {
  ...
  fastify.get<{ Params: { id: string } }>('/emails/:id', async (req, reply) => {
  ...
  fastify.post<{ Body: Pick<Email, 'to' | 'cc' | 'bcc' | 'subject' | 'body'> }>(
    '/emails',
    async (req, reply) => {
  ...
  fastify.put<{ Params: { id: string }; Body: Partial<Email> }>(
    '/emails/:id',
    async (req, reply) => {
      const id = Number(req.params.id);
  ...
  fastify.delete<{ Params: { id: string } }>('/emails/:id', async (req, reply) => {
    const id = Number(req.params.id);
  ...
  fastify.post<{ Body: { prompt: string; context?: Record<string, any> } }>(
    '/ai/draft',
    async (req, reply) => {
      ...
      const type: 'sales' | 'followup' =
        /follow\s*-?\s*up|checking in|touching base|ping/i.test(lower) ? 'followup' : 'sales';
  ...
  fastify.get<{ Querystring: { prompt: string } }>(
    '/ai/draft/stream',
    async (req, reply) => {
      const prompt = req.query.prompt;
      if (!prompt) return reply.code(400).send('prompt required');
...
function generateEmail(
  type: 'sales' | 'followup',
  prompt: string,
  context: { recipient?: string; business?: string }
): { subject: string; body: string } {
  if (type === 'followup') {
    const subject = trimToWordLimit(`Following up: ${prompt}`, 10);
    const body = trimToWordLimit(
      `Hi there, just checking in on ${prompt}. Do you have a minute this week? Happy to share details or adjust timing. Thanks!`,
      40
    );
    return { subject, body };
  }
  ...
  const body = trimToWordLimit(
    `Hi ${context.recipient ?? 'there'}, noticed ${prompt}${biz}. 1) Fast setup 2) Clear results. Open to a 10‑min chat this week?`,
    40
  );
  return { subject, body };
}
